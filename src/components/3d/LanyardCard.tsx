...
import { useThree, useFrame, extend, ThreeElements } from '@react-three/fiber'
import { useGLTF, useTexture } from '@react-three/drei'
import { BallCollider, CuboidCollider, RigidBody, useRopeJoint, useSphericalJoint, RigidBodyProps, RapierRigidBody } from '@react-three/rapier'
...
declare global {
  namespace JSX {
    interface IntrinsicElements {
      ...
      // Use any for now to bypass complex type issues, can refine later if needed
...
export default function LanyardCard({
  frontImage = '/assets/default_front.jpg',
  backImage = '/assets/default_back.jpg',
  ...
  useEffect(() => {
    if (cardBackRef.current) {
      cardBackRef.current.rotation.y = Math.PI;
    }
  }, [])
  ...
  useSphericalJoint(j3 as React.RefObject<RapierRigidBody>, card as React.RefObject<RapierRigidBody>, [[0, 0, 0], [0, 1.45, 0]])
  ...
  const handleMouseMove = (e: any) => {
    if (dragged && card.current) {
      ...
      vec.copy(e.point).sub(dragged as THREE.Vector3)
  ...
  const flipCard = () => {
    if (!isFlipping) {
      ...
      setTargetRotation(targetRotation === 0 ? Math.PI : 0)
    }
  }
  ...
  return (
    <>
      <group position={[position[0], 4, position[2]]}>
        <RigidBody ref={fixed as React.MutableRefObject<RapierRigidBody>} {...segmentProps} type="fixed" />
        <RigidBody position={[0.5, 0, 0]} ref={j1 as React.MutableRefObject<RapierRigidBody>} {...segmentProps}><BallCollider args={[0.1]} /></RigidBody>
        <RigidBody position={[1, 0, 0]} ref={j2 as React.MutableRefObject<RapierRigidBody>} {...segmentProps}><BallCollider args={[0.1]} /></RigidBody>
        <RigidBody position={[1.5, 0, 0]} ref={j3 as React.MutableRefObject<RapierRigidBody>} {...segmentProps}><BallCollider args={[0.1]} /></RigidBody>
        <RigidBody position={[2, 0, 0]} ref={card as React.MutableRefObject<RapierRigidBody>} {...segmentProps} type={dragged ? 'kinematicPosition' : 'dynamic'}>
